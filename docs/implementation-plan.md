...
- Pin the Stripe SDK version in `package.json` and document the configured Stripe API/webhook version in `README.md`; upgrade deliberately with webhook regression tests.
...
The main critique is that it's too detailed for its stated purpose. A reusable starter template's planning doc should be a fast onboarding reference, not a specification you need to read cover-to-cover before touching the code. Consider splitting it: keep §17 (decisions) and §3 (schema) in docs/, move the prose rationale into README.md sections, and let CLAUDE.md carry the conventions that Claude Code actually reads.

- Shortened and simplified section headings.
- Removed the premature `getUserBillingOwnerId` abstraction from the core plan.
- Kept explicit documentation that future organization billing requires a schema migration, data migration, and RLS rewrite.
- Simplified the billing-owner decision record accordingly.
- Retained the atomic Stripe event claim and `processing` / `processed` / `failed` semantics.
- Clarified that a concurrent `processing` event must not be acknowledged as successfully processed.
- Strengthened local Supabase RLS testing in the implementation sequence.
- Added Stripe SDK and API/webhook-version pinning guidance.
- Kept the lazy Stripe Customer design and documented orphan cleanup as an accepted edge case.
- Removed the empty “Open Issues” section because no genuinely unresolved core decision remains.
- Preserved the original decisions: Vitest plus Playwright, no live Stripe E2E in core, lazy Customers, password-only UI, user-owned billing, and manual Supabase migrations.
